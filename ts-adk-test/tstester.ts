// adkClient.ts
// Node 18+ (global fetch). If you're on older Node, `npm i node-fetch` and import it.
...
/** Call your ADK app with free-form text like "point A to point B" and return the assistant’s reply. */
export async function ask(text: string): Promise<string> {
  ...
  return (
    pickAssistantText(events) ||
    pickToolError(events) ||
    "(no assistant text)"
  );
}
