#!/usr/bin/env python3
# utils/agent1_utils.py
#
# Single-call tool that generates up to N distinct walking routes using
# Google Routes API, then returns JSON with encoded polylines + clickable links.
#
# Requirements:
#   pip install httpx "httpx[http2]" google-adk-agents
#
# Env:
#   export MAPS_KEY=YOUR_GOOGLE_MAPS_API_KEY  (Routes + Places enabled, billing on)
...
# --------------- Retry / Backoff ----------------------
def _sleep_time(attempt: int, base: float = 1.0, cap: float = 16.0) -> float:
    ...
    while True:
        try:
            ...
            if code in RETRY_STATUSES and attempt < max_tries - 1:
                time.sleep(_sleep_time(attempt))
                attempt += 1
                continue
            raise
...
def resolve_place(query: str) -> Tuple[str, Tuple[float, float]]:
    """Return (place_id, (lat, lng)) for a text query with Miami bias."""
    if not MAPS_KEY:
...
def _build_candidates(
    ...
    cands: List[List[Tuple[float, float]]] = [[]]  # baseline only
    ...
    # 2-waypoint origin arc + dest arc
...
# ---------------- Core single tool -------------------
def plan_routes(origin: str, destination: str, count: int = 10, output: str = "json"):
    """
    ...
    """
    if not MAPS_KEY:
        return {"status": "error", "error_message": "Missing MAPS_KEY"}
    ...
    try:
        ...
        return {"status": "error", "error_message": f"Baseline failed: {e}"}
    ...
    if origin_ll is None or dest_ll is None:
        ...
        if len(pts0) >= 2:
            ...
            return {"status": "error", "error_message": "Could not infer coordinates from baseline polyline"}
    ...
    # Always return JSON for chaining
    if not kept:
        return {"status": "error", "error_message": f"No routes found for {origin} -> {destination}"}
    return {
        "status": "success",
        "origin": origin,
